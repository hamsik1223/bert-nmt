...
class FlatTransformer_SE_Model(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        return FlatTransformer_SE_Model(encoder, decoder, bertencoder, src_berttokenizer, args.mask_cls_sep, args)
    ...
    #):
        ...
        bert_encoder_out, _ =  self.bert_encoder(bert_input, output_all_encoded_layers=True, attention_mask= bert_encoder_padding_mask.logical_not())
...
class FlatTransformer_SE_Encoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.sen_layers = nn.ModuleList([])
        self.sen_layers.extend(
            [self.build_encoder_layer(args) for i in range(args.encoder_bot_layers+args.encoder_top_layers)]
        )

        if args.encoder_normalize_before:
            self.layer_norm = nn.ModuleList([])
            self.layer_norm.extend(
            [LayerNorm(embed_dim) for i in range(args.encoder_bot_layers+args.encoder_top_layers+1)]
        )
        else:
        ...
        if getattr(args, "layernorm_embedding", False):
            self.layernorm_embedding_se = LayerNorm(embed_dim)
        else:
            self.layernorm_embedding_se = None


        self.linear_gate_local = nn.ModuleList([])
        self.linear_gate_local.extend(
            [Linear(embed_dim, 1, bias=True) for i in range(args.encoder_bot_layers+args.encoder_top_layers)]
        )

        self.linear_gate_global = nn.ModuleList([])
        self.linear_gate_global.extend(
            [Linear(embed_dim, 1, bias=True) for i in range(args.encoder_bot_layers+args.encoder_top_layers)]
        )
    ...
    def forward_embedding_sentences(self, src_sentences, embed_sentences):
        ...
        ###
        if self.layernorm_embedding_se is not None:
            x = self.layernorm_embedding_se(x)
        x = F.dropout(x, p=self.dropout, training=self.training)
        if self.quant_noise is not None:
            x = self.quant_noise(x)
        return x, embed
    ...
    ###
    def enc_to_dec_prepare(self, x_sen, src_sentences, x, src_tokens, encoder_padding_mask, layer):
        '''
        ...
        # 2. add x and x_sen_current_h
        x_sen_current_h = x_sen_current_h.repeat([x.size()[0],1,1])
 
        #x = torch.cat([x_sen_current_h, x], dim = -1) 

        x_encoded = self.linear_gate_local[layer](x)
        x_sen_current_h_encoded = self.linear_gate_global[layer](x_sen_current_h)

        alpha = torch.sigmoid(x_encoded+x_sen_current_h_encoded).repeat([1,1,x.size()[2]])
        x = x * alpha + (1-alpha) * x_sen_current_h
    ...
    def forward(
        ...
        Args:
            ...
            src_lengths (torch.LongTensor): lengths of each source sentence of
                shape `(batch)`
            bert_encoder_out['bert_encoder_out'], bert_encoder_out['bert_encoder_padding_mask']
        ...
        ### with different mask 
        # for token layers
        for layers in range(self.bot_layers + self.top_layers):
            layer_for_token = self.layers[layers]
            x,_ = layer_for_token(x, encoder_padding_mask)
            if return_all_hiddens: 
                assert encoder_states is not None
                encoder_states.append(x)

            layer_for_sen = self.sen_layers[layers]
            x_sen,_ = layer_for_sen(x_sen, encoder_sen_padding_mask)
            if return_all_hiddens:
                assert encoder_states is not None
                encoder_states.append(x_sen)

            if layers >= self.bot_layers:
                if self.layer_norm is not None:
                    x = self.layer_norm[layers](x)
                if self.layer_norm is not None:
                    x_sen = self.layer_norm[layers](x_sen)

                x = self.enc_to_dec_prepare(x_sen, src_sentences, x, src_tokens, encoder_padding_mask, layers)
    ...
    def max_positions(self):
        ...
        ###yufeng ma: for bert-nmt, need to use self.embed_positions.max_positions() instead of self.embed_positions.max_positions
